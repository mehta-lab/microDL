...
class MaskProcessor:
    ...
    def _get_flat_field_paths(self, channel_ids):
        """
        ...
        if not isinstance(channel_ids, list):
            channel_ids = [channel_ids]
        for channel_idx in channel_ids:
            flat_field_path = im_utils.get_flat_field_path(
                self.flat_field_dir,
                channel_idx,
                channel_ids,
            )
            flat_field_fnames.append(flat_field_path)
    ...
    def generate_masks(self, str_elem_radius=5):
        ...
        if self.uniform_struct:
            for id_row in id_df.to_numpy():
                ...
                flat_field_fnames = self._get_flat_field_paths(
                    channel_ids=self.channel_ids,
                )
                flat_field_fnames = self._get_flat_field_paths(
                    channel_ids=self.channel_ids,
