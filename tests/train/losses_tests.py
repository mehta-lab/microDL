...
def test_mae_loss():
    ...
    mae = losses.mae_loss(y_true, y_pred, mean_loss=False)
...
def test_mse_loss():
    ...
    y_true = tf.convert_to_tensor(y_true, dtype=tf.float32)
    y_pred = tf.convert_to_tensor(y_pred, dtype=tf.float32)
    mse = losses.mse_loss(y_true, y_pred, mean_loss=False)
    with tf.Session() as sess:
        res = sess.run(mse)
...
def test_binary_crossentropy_loss():
    ...
    binary_crossentropy_loss = \
        losses.binary_crossentropy_loss(y_true, y_pred, mean_loss=False)
    with tf.Session() as sess:
