...
class TestImage2DToVectorNet(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.network_config = {'num_input_channels': 4,
        ...
        self.net = Image2DToVectorNet(self.network_config)
        inputs, outputs = self.net.build_net()
        self.model = Model(inputs, outputs)
        self.model_layers = self.model.layers

    def test_ImageToVectorNet_init(self):
        """Test initialization"""

        # check if both num_inital_filters and num_filters_per_block provided
        self.network_config['num_filters_per_block'] = [16, 32, 48, 64]
        nose.tools.assert_raises(AssertionError,
                                 Image2DToVectorNet,
                                 self.network_config)
        self.network_config.pop('num_filters_per_block')

        # check for dense_type
        self.network_config['dense']['type'] = 'pool'
        nose.tools.assert_raises(AssertionError,
                                 Image2DToVectorNet,
                                 self.network_config)
        self.network_config['dense']['type'] = 'dense'

        # check for drop_out with 1x1 conv, res_pool_type
        ...
        # check for height and width not being powers of 2
        self.network_config['width'] = 48
        nose.tools.assert_raises(AssertionError,
                                 Image2DToVectorNet,
                                 self.network_config)
        self.network_config['width'] = 64

    def test_ImageToVectorNet_get_input_shape(self):
    ...
    def test_ImageToVectorNet_init_filters_res_shapes(self):
    ...
    def test_ImageToVectorNet_filters_per_block_shapes(self):
        """Test for intermediate shapes"""
        ...
        self.network_config['dense']['num_units'] = [64, 32]
        self.network_config['dense']['type'] = 'conv'
        self.net = Image2DToVectorNet(self.network_config)
        inputs, outputs = self.net.build_net()
        self.model = Model(inputs, outputs)
    ...
    # skipping the test for all weights getting trained. The weights don't
    # seem to budge when trying to predict random vectors or vector of ones at
    # various learning rates! Makes sense, what scalar/vector can be predicted
