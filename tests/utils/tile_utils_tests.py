...
def _get_name(ch_idx, sl_idx, time_idx, pos_idx):
    im_name = 'im_c' + str(ch_idx).zfill(int2str_len) + \
              '_z' + str(sl_idx).zfill(int2str_len) + \
              '_t' + str(time_idx).zfill(int2str_len) + \
              '_p' + str(pos_idx).zfill(int2str_len) + ".png"
    return im_name


for z in range(sph.shape[2]):
    im_name = _get_name(1, z, time_ids, pos_ids)
    sk_im_io.imsave(os.path.join(temp_path, im_name),
                    sph[:, :, z])
...
# Write metadata
frames_meta.to_csv(os.path.join(temp_path, meta_fname), sep=',')


def test_read_imstack():
    """Test read_imstack"""

    fnames = frames_meta['file_name'][:3]
    fnames = [os.path.join(temp_path, fname) for fname in fnames]
    # non-boolean
    im_stack = tile_utils.read_imstack(fnames)
    exp_stack = zscore(sph[:, :, :3])
    numpy.testing.assert_equal(im_stack.shape, (32, 32, 3))
    numpy.testing.assert_array_equal(exp_stack[:3], im_stack)


def test_preprocess_imstack():
    """Test preprocess_imstack"""

    im_stack = tile_utils.preprocess_imstack(frames_meta,
                                             temp_path,
                                             depth=3,
                                             time_idx=time_ids,
...
def test_tile_image():
    ...
    for row in range(0, 17, 8):
        for col in range(0, 17, 8):
            id_str = 'r{}-{}_c{}-{}_sl{}-{}'.format(row, row + tile_size[0],
                                                    col, col + tile_size[1],
                                                    0, 3)
            nose.tools.assert_equal(id_str, tiled_image_list[c][0])
            tile = input_image[row:row + tile_size[0], col: col + tile_size[1],
                   ...]
    ...
    tile_dir = os.path.join(temp_path, 'tile_dir')
    os.makedirs(tile_dir, exist_ok=True)
    meta_dir = os.path.join(tile_dir, 'meta_dir')
    os.makedirs(meta_dir, exist_ok=True)
    save_dict = {'time_idx': time_ids,
                 'channel_idx': channel_ids,
                 'slice_idx': 4,
                 'pos_idx': pos_ids,
                 'image_format':'zyx',
    ...
    for row in range(0, 17, 8):
        for col in range(0, 17, 8):
            id_str = 'r{}-{}_c{}-{}_sl{}-{}'.format(row, row + tile_size[0],
                                                    col, col + tile_size[1],
                                                    0, 3)
            cur_fname = aux_utils.get_im_name(time_idx=time_ids,
                                              channel_idx=channel_ids,
                                              slice_idx=4,
                                              pos_idx=pos_ids,
                                              int2str_len=3,
            ...
            cur_meta = {'channel_idx': channel_ids,
                        'slice_idx': 4,
                        'time_idx': time_ids,
                        'file_name': cur_fname,
                        'pos_idx': pos_ids,
...
def write_meta():
    ...
    tile_meta = [{'channel_idx': channel_ids,
                  'slice_idx': 4,
                  'time_idx': time_ids,
